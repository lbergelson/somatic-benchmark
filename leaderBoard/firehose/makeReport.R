...
stats_summary <- ddply(stats, .(EvaluationGroup, Caller,Version),
	 function(df) c(fp_snp = mean(df$fp_snp), tp_snp = mean(df$tp_snp), fn_snp = mean(df$fn_snp), novel_snp = mean(df$novel_snp),
		fp_indel = mean(df$fp_indel), tp_indel = mean(df$tp_indel), fn_indel = mean(df$fn_indel), novel_indel = mean(df$novel_indel) , Samples = nrow(df)))
...
common_cols <- c("Caller", "Version", "Samples" )
...
stat_1954_s <- stats_summary[grepl("HCC1954",stats_summary$EvaluationGroup) ,snp_cols]
stat_1954_i <- stats_summary[grepl("HCC1954",stats_summary$EvaluationGroup),indel_cols]
